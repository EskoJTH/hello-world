...
data Rock = Rock PointInSpace Size Velocity deriving (Eq,Show) --lisösin uuden ufo datatyypin.
...
simulateWorld timeStep (Play rocks (Ship shipPos shipV) bullets) --lisäsin tälle inputiks myös ufon tuonne sekaan
  ...
  where
    ...
    collidesWith p (Rock rp s _) = magV (rp .- p) < s 
                                   ...
                                   --Lisäsin vielä toisen Collision detectorin Ufole tähän jostain syystä
