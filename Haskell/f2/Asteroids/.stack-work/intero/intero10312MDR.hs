...
data Action = Scan [Rock] PointInSpace Int |Move|Shoot Rock Int deriving (Eq,Show)
...
simulateWorld timeStep (Play rocks (Ship shipPos shipV) bullets ufo scanner) --lisäsin tälle inputiks myös ufon tuonne sekaan
  ...
  where
    ...
    doUfoAction (UFO point size v (Phase (x, xs))) = case x of
      Scan rocks direction timer-> showScanVisualizer point direction
      Move -> undefined
      Shoot rock timer -> case timer of
        waitAfterShot -> (UFO point size v xs)
        _ -> (UFO point size v (Phase ((Shoot rock (timer+1)),xs)))
        
    didUfoShoot :: UFO -> [Bullet]->[Bullet]
    didUfoShoot (UFO point size v (Phase (x, xs))) bullets = case x of
      Shoot (Rock locationR sizeR vR) 0 ->((Bullet point (-150 .* norm (point .- newRockLocation))) 0 ):bullets
      _ -> bullets
      where
        newRockLocation = (bulletVelocity .* (norm (locationR .- point))) .+ (speedR)

     scanner (UFO point size v (Phase (x, xs))) bullets = 
